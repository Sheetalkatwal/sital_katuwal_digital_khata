from accounts.models import MyUser, UserProfile, Business,Otp,CustomerRequest,Customer,Notification
...
class BusinessSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Business
        fields = ['id', 'business_name', 'owner_name', 'connection_status', 'lat', 'lng', 'delivery_radius_km']
...
class BusinessDeliverySettingsSerializer(serializers.ModelSerializer):
    delivery_radius_km = serializers.FloatField(required=True)

    class Meta:
        model = Business
        fields = ['id', 'business_name', 'lat', 'lng', 'delivery_radius_km']
        read_only_fields = ['id', 'business_name']

    def validate_delivery_radius_km(self, value):
        if value is None:
            raise serializers.ValidationError("Delivery radius is required.")
        if value < 1 or value > 25:
            raise serializers.ValidationError("Delivery radius must be between 1 km and 25 km.")
...
class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification
        fields = ['id', 'title', 'message', 'category', 'payload', 'is_read', 'created_at', 'read_at']
